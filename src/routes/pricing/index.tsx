  ...
  return (
    <>
      <section class="container flex flex-col  gap-6 py-8 md:max-w-[64rem] md:py-12 lg:py-24">
        ...
        {list.map((item) => {
          return <Card key={item.title} title={item.title} cost={item.cost} />;
        })}
...
const Card = component$(({ title, cost }: { title: string; cost: number }) => {
  return (
    <div class="grid w-full items-start gap-10 rounded-lg border p-10 md:grid-cols-[1fr_200px]">
      <div class="grid gap-6">
        ...
        <ul class="grid gap-3 text-sm text-muted-foreground sm:grid-cols-2">
          ...
          <li class="flex items-center">
            <Icon clsx="mr-2 h-4 w-4" name="Check" /> Dashboard Analytics
          </li>
          <li class="flex items-center">
      ...
      <div class="flex flex-col gap-4 text-center">
        <div>
          <h4 class="text-7xl font-bold">${cost}</h4>
